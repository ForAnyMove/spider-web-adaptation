...
screenParameters.openCallback = launchPlaygroundScene;

function launchPlaygroundScene() {
  enableInteractions();
  const homeButtons = document.getElementById('playground-screen').querySelectorAll('.home-btn');

  for (let i = 0; i < homeButtons.length; i++) {
    const element = homeButtons[i];
    element.onclick = navigateToMainMenu;
  }
  ...
  const settingsRoot = document.getElementById('settings-game');
  ...
  const languageScreen = new Screen({
    ...
    openButtons: document.getElementsByClassName('lang-btn'),
...
`;
  ...
  function startCurrentLevel() {
    ...
    const cardLockers = document.getElementsByClassName('card-locked-showed');

    for (let i = cardLockers.length - 1; i >= 0; i--) {
      const element = cardLockers[i];
      element.remove();
    }
  ...
  function setupBackgroundChange() {
    ...
    const backgroundImage = document.getElementById('playground-screen');
  ...
  function setupButtons() {
    ...
    const itemListUpdateListener_01 = () => {
    ...
    user.itemListUpdateEvent.addListener(itemListUpdateListener_01)
  ...
  function subscribeFinishScreenRewardsObtain() {
    ...
    user.onItemsPublicReceive.addListener(getLevelReward);
  ...
  function invokeTutorial() {
    const isTutorial = levelParameter.includes('isTutorial'); //isTutorial
...
function clear() {
  const elements = Array.from(document.getElementsByClassName('card-locked-showed'))
    .concat(Array.from(document.getElementsByClassName('card-locked-hidden')))
    .concat(Array.from(document.getElementsByClassName('card-element')));
...
function unload() {
  ...
  levelParameter = null;
  user.contentUsageChanged.removeListenerByName('updateBackground');
  user.contentUsageChanged.removeListenerByName('updateSkin');
  user.onItemsPublicReceive.removeListenerByName('getLevelReward');
  user.itemListUpdateEvent.removeListenerByName('itemListUpdateListener_01');
...
export { setupLanguageSelector, screenParameters }